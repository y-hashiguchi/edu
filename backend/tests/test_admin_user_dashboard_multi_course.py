...
async def test_admin_can_view_any_course_dashboard_without_enrollment(
    ...
    await _seed(db_session, auth_user.id)

    client.headers.update({"Authorization": f"Bearer {admin_token}"})
    res = client.get(f"/api/admin/users/{auth_user.id}/dashboard?course=ai-era-se")
    assert res.status_code == 200


@pytest.mark.asyncio
async def test_admin_dashboard_unknown_course_returns_404(client, auth_user, admin_token):
    client.headers.update({"Authorization": f"Bearer {admin_token}"})
    res = client.get(f"/api/admin/users/{auth_user.id}/dashboard?course=nope")
    assert res.status_code == 404


@pytest.mark.asyncio
async def test_admin_dashboard_default_course_when_param_missing(client, auth_user, admin_token):
    client.headers.update({"Authorization": f"Bearer {admin_token}"})
