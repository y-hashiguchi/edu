...
1. **MED-3**（情報漏えい）— ユーザー可視メッセージなので体験にも影響。最優先。
2. **MED-4**（ファイル名衝突）— 再現可能なバグなので Sprint 4 早期に。
3. **MED-2**（grading の root チェック）— 攻撃難易度は高いが防御層の統一なので一緒に。
4. **MED-1**（XML 区切り）— LLM 防御の改善。
5. **MED-5**（CSP）— 本番化フェーズと併せて。
6. **LOW-2, LOW-3** — Sprint 4 のリファクタ余裕枠で。
