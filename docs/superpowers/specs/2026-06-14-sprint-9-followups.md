...
## LOW-2: Multi-worker 環境での cache 不整合 — **2026-06-11 完了**

**Fix:** `app/services/curriculum_cache_pubsub.py` — publish 後に Redis pub/sub で slug を broadcast。各 worker の lifespan listener が `reload_course` / `reload_from_db` を実行。`curriculum_cache_pubsub_enabled` で無効化可（テストは false）。
...
| ID | Severity | Source | Owner | 状態 | コミット |
|----|----------|--------|-------|------|---------|
| MED-1 | M | code-reviewer | controller | done | `cb9276b` |
| MED-2 | M | code-reviewer | controller | done | `cb9276b` |
| MED-3 | M | security-reviewer | controller | done | `3f9cc41` |
| MED-4 | M | code-reviewer | controller | done | `45bc37c` |
| MED-5 | M | security-reviewer | controller | done (doc only) | `c40cd73` |
| LOW-1 | L | security-reviewer | controller | done | `3f9cc41` |
| LOW-2 | L | security-reviewer | controller | done | Sprint 12 |
| LOW-3 | L | code-reviewer | controller | done | `c40cd73` |
| LOW-4 | L | code-reviewer | controller | done | `76b524c` |

HIGH × 3 (publish cache-after-commit / audit log / route param `:courseSlug`) は同 sprint 内コミット `854a252` で対応済み。MED × 5 + LOW × 3 を Sprint 9 follow-up branch (`feature/sprint-9-followups`) で消化。残るは LOW-2 (multi-worker cache 不整合) のみ、Sprint 10+ での horizontal scale 時に対応。
